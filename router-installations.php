...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  #techName::placeholder { color: rgba(255,255,255,0.4); }
  ...
  .progress-label { font-size: 13px; color: rgba(255,255,255,0.8); }
  ...
  .count-online { color: #6EE7A0; font-weight: 600; }
  .count-issue  { color: #FCA5A5; font-weight: 600; }
  .count-pending{ color: rgba(255,255,255,0.55); }
  ...
  /* Note: filters-bar is not sticky relative to header — we'll use JS for offset */
  .filter-group {
    display: flex;
    gap: 7px;
    flex-wrap: wrap;
  ...
  .filter-pill:hover { background: var(--accent-light); border-color: var(--accent); color: var(--accent); }
  ...
  .filter-pill.active-online {
    background: var(--green);
    border-color: var(--green);
    color: white;
  }

  .filter-pill.active-issue {
    background: var(--red);
  ...
  .status-online .card-status-bar  { background: var(--green); }
  ...
  .outcome-chip.online { background: var(--green-bg); color: var(--green); border: 1px solid var(--green-border); }
  .outcome-chip.issue  { background: var(--red-bg); color: var(--red); border: 1px solid var(--red-border); }
  .outcome-chip.pending{ background: var(--bg); color: var(--text-light); border: 1px solid var(--border); }
  ...
  .site-card.expanded .expand-icon { transform: rotate(180deg); }
  ...
  .site-card.expanded .card-body { display: block; }
  ...
  .detail-value.highlight-bad {
    color: var(--red);
    font-weight: 600;
  }

  .detail-value.highlight-warn {
    color: var(--amber);
    font-weight: 600;
  }
  ...
  .issue-note-wrap.visible { display: flex; }
  ...
  .issue-textarea::placeholder { color: var(--text-light); }
  ...
  .outcome-recorded.online {
    display: flex;
    background: var(--green-bg);
    border: 1px solid var(--green-border);
  }

  .outcome-recorded.issue {
    display: flex;
    background: var(--red-bg);
    border: 1px solid var(--red-border);
  }

  .outcome-icon { font-size: 20px; line-height: 1; flex-shrink: 0; }
  ...
  .online .outcome-title { color: var(--green); }
  .issue  .outcome-title { color: var(--red); }
  ...
  .copy-success.visible { display: flex; }
  ...
  @media (max-width: 600px) {
    ...
    .filter-pill { font-size: 13px; padding: 6px 11px; }
    .sites-container { padding: 6px 8px 80px; }
  }

  @media (min-width: 768px) {
    .header { padding: 20px 28px 16px; }
    .filters-bar { padding: 10px 24px; }
    .sites-container { padding: 12px 24px 80px; }
    .toolbar { padding: 14px 24px 8px; }
...
<header class="header" id="mainHeader">
  ...
  <div class="header-meta">
    ...
    <div class="tech-input-wrap">
      ...
      <input type="text" id="techName" placeholder="Enter your name" autocomplete="off" autocorrect="off" spellcheck="false">
    </div>
  </div>
  ...
  <div class="progress-bar-section">
    ...
    <div class="progress-track">
      <div class="progress-fill-online" id="progressOnline" style="width:0%"></div>
      <div class="progress-fill-issue"  id="progressIssue"  style="width:0%"></div>
    </div>
  </div>
</header>
...
<div class="filters-bar" id="filtersBar">
  <div class="filter-group" id="districtFilters">
    ...
    <button class="filter-pill" data-filter-type="district" data-filter-value="South Tyneside">S. Tyneside <span class="pill-count" id="cnt-dist-SouthTyneside">0</span></button>
    <button class="filter-pill" data-filter-type="district" data-filter-value="Sunderland">Sunderland <span class="pill-count" id="cnt-dist-Sunderland">0</span></button>
    <button class="filter-pill" data-filter-type="district" data-filter-value="Northumberland">Northumberland <span class="pill-count" id="cnt-dist-Northumberland">0</span></button>
...
<!-- ═══ TOOLBAR ═══ -->
<div class="toolbar">
  <button class="btn btn-primary" onclick="openReportModal()" aria-label="Generate report">
    &#128196; Generate Report
  </button>
  <button class="btn btn-outline" onclick="downloadCSV()" aria-label="Download CSV">
    &#128202; Download CSV
  </button>
  <button class="btn btn-danger" onclick="confirmClearAll()" aria-label="Clear all data">
    &#128465; Clear All Data
  </button>
</div>
  ...
  <div class="modal">
    ...
    <div style="display:flex; align-items:center; gap:12px; flex-wrap:wrap; justify-content:space-between;">
      ...
      <div class="modal-actions" style="display:flex; gap:8px; flex-wrap:wrap;">
        <button class="btn btn-primary" id="shareBtn" onclick="shareReport()" style="display:none;">&#8618; Share Report</button>
        <button class="btn btn-primary" onclick="emailReport()" style="background:#1B2A4A; border-color:#1B2A4A;">&#9993; Email Report</button>
        <button class="btn btn-outline" onclick="downloadReport()">&#8615; Download .txt</button>
        <button class="btn btn-outline" onclick="copyReport()">Copy</button>
...
<div class="modal-overlay" id="clearModal" role="dialog" aria-modal="true" aria-labelledby="clearModalTitle">
  <div class="modal">
    <h2 id="clearModalTitle">&#9888; Clear All Data?</h2>
    <div class="modal-body">
      This will permanently delete all recorded outcomes, issue notes, and the technician name from this device.
...
function updateDistrictCounts() {
  ...
  const allEl = document.getElementById('cnt-dist-All');
  if (allEl) allEl.textContent = total;

  ['Newcastle','North Tyneside','Gateshead','South Tyneside','Sunderland','Northumberland'].forEach(d => {
    const el = document.getElementById('cnt-dist-' + districtKey(d));
...
function renderSiteCard(s) {
  ...
  const notesBoxHtml = s.notes ? `
    <div class="notes-box">
      <div class="notes-box-label">Existing Notes (from spreadsheet)</div>
      ${escapeHtml(s.notes)}
    </div>` : '';
...
<article class="site-card ${statusCls}" id="card-${s.id}" data-id="${s.id}" data-district="${escapeHtml(s.district)}" role="article" aria-label="${escapeHtml(s.scn)} ${escapeHtml(s.site)}">
  <div class="card-header" onclick="toggleCard(${s.id})" role="button" tabindex="0"
    ...
    <div class="card-header-main">
      <div class="card-top-row">
        <span class="scn-label">${escapeHtml(s.scn)}</span>
        <span class="district-badge district-${distKey}">${escapeHtml(s.district)}</span>
      </div>
  ...
  <div class="card-body" id="body-${s.id}" role="region" aria-labelledby="header-${s.id}">
    ...
    ${s.lat ? `<div class="nav-btn-wrap">
      <a class="nav-btn" href="https://www.google.com/maps/dir/?api=1&destination=${s.lat},${s.lon}" target="_blank" rel="noopener" onclick="event.stopPropagation()">
    ...
    <div class="action-area" id="action-${s.id}">
      ...
      <div class="confirm-bar" id="confirm-wrap-${s.id}">
        <span class="confirm-msg">Mark as installed?</span>
        <button class="confirm-yes" onclick="confirmOutcome(${s.id}, event)">Confirm</button>
        <button class="confirm-no" onclick="cancelConfirm(${s.id}, event)">Cancel</button>
      </div>
...
function renderAll() {
  ...
  if (filtered.length === 0) {
    ...
    container.innerHTML = filtered.map(renderSiteCard).join('');
  }
...
function selectOutcome(id, type, event) {
  ...
  const confirmWrap = document.getElementById('confirm-wrap-' + id);
...
function confirmOutcome(id, event) {
  event && event.stopPropagation();
...
function cancelConfirm(id, event) {
  event && event.stopPropagation();
  const onlineBtn   = document.getElementById('btn-online-' + id);
  const confirmWrap = document.getElementById('confirm-wrap-' + id);
  onlineBtn && onlineBtn.classList.remove('selected');
...
function saveIssue(id) {
  ...
  if (!note) {
    ta && ta.focus();
    ta && (ta.style.borderColor = 'var(--red)');
    ta && ta.setCustomValidity('Please describe the issue.');
    ta && ta.reportValidity();
    return;
  }
...
function refreshCardOutcome(id) {
  ...
  // For status-filter edge cases we do a full render, otherwise surgical update
  ...
  const newHtml = renderSiteCard(site);
...
function updateFilterPillStates() {
  document.querySelectorAll('[data-filter-type]').forEach(btn => {
    ...
    btn.classList.remove('active', 'active-online', 'active-issue');
    if (type === 'district' && value === activeDistrictFilter) {
      btn.classList.add('active');
    } else if (type === 'status') {
      if (value === activeStatusFilter) {
        if (value === 'online') btn.classList.add('active-online');
        else if (value === 'issue') btn.classList.add('active-issue');
...
document.getElementById('districtFilters').addEventListener('click', function(e) {
  const btn = e.target.closest('[data-filter-type]');
  if (!btn) return;
  const type  = btn.dataset.filterType;
  const value = btn.dataset.filterValue;
  if (type === 'district') activeDistrictFilter = value;
  if (type === 'status')   activeStatusFilter   = value;
  renderAll();
});
...
function openReportModal() {
  ...
  const bySite = sites.reduce((acc, s) => {
...
function copyReport() {
  ...
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(ta.value).then(() => {
      ...
      setTimeout(() => document.getElementById('copySuccess').classList.remove('visible'), 3000);
    });
  } else {
    ta.select();
...
/* ══════════════════════════════════════════
   STICKY FILTER BAR — offset below header
══════════════════════════════════════════ */
function setFilterBarOffset() {
...
window.addEventListener('resize', setFilterBarOffset);
...
function init() {
  ...
  // Set filter bar offset after render
  requestAnimationFrame(setFilterBarOffset);

  // Then load from API and re-render if newer data exists
  loadFromApi().then(loaded => { if (loaded) renderAll(); });
}
